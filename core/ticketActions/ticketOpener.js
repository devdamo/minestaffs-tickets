...
const roleGiverSystem = require('../interactionHandlers/buttons/roleGiver')
        ...
        if (configParser.getTicketValuesArray("id").includes(customId)){
            ...
            if (config.system.maxAmountOfTickets == 0 || !currentUserTicketAmount || currentUserTicketAmount == "false" || Number(currentUserTicketAmount) < config.system.maxAmountOfTickets){
                ...
                interaction.guild.channels.create({
                    ...
                    // Create components array with close button
                    const components = [closeButton]
                    
                    // Add role giver buttons if configured for this ticket type
                    if (currentTicketOptions.roleGivers && currentTicketOptions.roleGivers.length > 0) {
                        const roleGiverRow = roleGiverSystem.createRoleGiverButtons(currentTicketOptions.roleGivers)
                        if (roleGiverRow) {
                            components.push(roleGiverRow)
                        }
                    }
                    ...
                    // Log the event to the specified user
                    eventLogger.logTicketCreated(interaction.user, ticketChannel, interaction.guild, currentTicketOptions.name)
                    ...
                    try{
                        if (currentTicketOptions.enableDmOnOpen) {
                            interaction.member.send({embeds:[bot.errorLog.custom(l.messages.newTicketDmTitle,currentTicketOptions.message,":ticket:",config.color)],components:[channelbutton]})
                                .catch(error => {
                                    // Only log if it's not a DM permissions error
                                    if (error.code !== 50007) {
                                        log("system","failed to send DM: " + error.message)
                                    }
                                })
                        }
                    }
                    ...
                    if (config.system.dmMessages){
                        interaction.member.send({embeds:[bot.errorLog.warning(l.errors.maxAmountTitle,l.errors.maxAmountDescription)]})
                            .catch(error => {
                                // Only log if it's not a DM permissions error
                                if (error.code !== 50007) {
                                    log("system","can't send DM to member: " + error.message)
                                }
                            })
                    }
                }
